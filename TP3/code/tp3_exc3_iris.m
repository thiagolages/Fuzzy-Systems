...
type = 'gaussmf';
numMFs = repmat([n],1,4);
mfType = char(type,type,type,type);
...
maxIter = 100;
[fis2, trainError] = anfis([X(1:105,:) Yd(1:105)], fis2, maxIter);
disp("Terminei ANFIS");
% Evaluate FIS on Training data 
Ys = evalfis(X(1:105,:), fis2);
% YsOut = thfunc(Ys, 0.5); 
YsOut = Ys >= 0.5; 
accuracy = length(find(Yd(1:105)==YsOut))/length(Yd(1:105)) % accuracy
 disp(n); % numero de MFs
 out_acc(i) = accuracy;
 out_n(i) = n;
 
 % Evaluate FIS on TESTING data 
Ys = evalfis(X(106:end,:), fis2);
% YsOut = thfunc(Ys, 0.5); 
YsOut = Ys >= 0.5; 
accuracy = length(find(Yd(106:end)==YsOut))/length(Yd(106:end)); % accuracy
